  ...
  return (
    <>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, background: '#f9fafb', border: '1px solid #e5e7eb', borderRadius: 8, padding: '12px 16px', marginBottom: 12, fontFamily: 'monospace', fontSize: 16, wordBreak: 'break-all' }}>
        ...
        <button type="button" onClick={handleCopy} title="Copy recovery key" style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 4, color: copied ? '#10b981' : '#6b7280', flexShrink: 0 }}>
          {copied ? <Check size={18} /> : <Copy size={18} />}
        </button>
      </div>
      {copied && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, background: '#fef3c7', border: '1px solid #fde68a', borderRadius: 8, padding: '8px 12px', marginBottom: 12, color: '#92400e', fontSize: 13 }}>
